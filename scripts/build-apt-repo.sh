...
echo "  npm run sync:apt-repo:r2   # or: R2_BUCKET=foldops-apt ./scripts/upload-apt-repo-r2.sh"
echo ""
echo "On farm nodes (/etc/apt/sources.list.d/foldops.list) — use signed repo in production:"
echo "  deb [signed-by=/usr/share/keyrings/foldops.gpg] https://deb.folding-os.com $SUITE $COMPONENT"
echo "  (unsigned test only: deb [trusted=yes] https://deb.folding-os.com $SUITE $COMPONENT)"
